...
RC RBFM_ScanIterator::getNextRecord(RID &rid, void *data){
    ...
    if(returnVal != RBFM_EOF){
        ...
        // rbfm->readRecord(fileHandle, recordDescriptor, rid, tempRecordData);
        // rbfm->printRecord(recordDescriptor,tempRecordData);
        // free(tempRecordData);
    }
...
int RBFM_ScanIterator::getResultData(void* recordData, void* data){
    ...
    for(unsigned i=0; i<attributeNames.size(); i++){
        // cout<<"attributeName = "<<attributeNames[i]<<endl;
        for(unsigned j=0; j<recordDescriptor.size(); j++){
            if(attributeNames[i] == recordDescriptor[j].name){
...
int RBFM_ScanIterator::concatenateResult(void* attrData, AttrType type, int &offset, int attrCount, void* data){
    ...
    if((*(char*)attrData ^ 0x80) == 0)
        ...
        switch(type){
            case TypeVarChar:{
                // int varLen = *(int*) ((char*)attrData + sizeof(char));
                // string str = string((char*)((char*)attrData + sizeof(char)+ sizeof(int)),varLen);
                // cout<<"attrData = "<<str<<endl;
                len = sizeof(int) + *(int*) ((char*)attrData + sizeof(char));
                break;
            }
            case TypeInt:
                len = sizeof(int);
                // cout<<"attrData = "<<*(int*)((char*)attrData+sizeof(char))<<endl;
                break;
            case TypeReal:
                len = sizeof(float);
                // cout<<"attrData = "<<*(float*)((char*)attrData+sizeof(char))<<endl;
                break;
        }
    }
...
bool RBFM_ScanIterator::checkRecordRequirement(void* pageData, void* recordData){
    // cout<<"In RBFM_ScanIterator::checkRecordRequirement--------------------"<<endl;
    RecordBasedFileManager *rbfm = RecordBasedFileManager::instance();
    ...
    // RecordBasedFileManager::instance()->readRecord(fileHandle, recordDescriptor, nextRID, recordData);
    // RecordBasedFileManager::instance()->printRecord(recordDescriptor,recordData);
    // cout<<"Leave RBFM_ScanIterator::checkRecordRequirement--------------------"<<endl;
    return true;
}
...
bool RBFM_ScanIterator::checkAttribute(void* value, void* attrData){
    ...
    switch (conditionAttributeType) {
        ...
        case TypeInt:
            ...
            break;
        case TypeReal:
            float temp = *((float*) ((char*)attrData + sizeof(char))) - *((float*) value);
            result = 0;
            if(temp < -0.000001)
                result = -1;
            if(temp > 0.000001)
